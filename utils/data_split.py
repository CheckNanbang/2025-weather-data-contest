...
def data_split(split_type, train_df, target_column):
    ...
    if split_type == "random":
        ...
        x_train = train_df[~((train_df["train_heattm"] >= holdout_start) & (train_df["train_heattm"] <= holdout_end))].drop(columns=[target_column])
        y_train = train_df[~((train_df["train_heattm"] >= holdout_start) & (train_df["train_heattm"] <= holdout_end))][target_column]

        x_valid = train_df[(train_df["train_heattm"] >= holdout_start) & (train_df["train_heattm"] <= holdout_end)].drop(columns=[target_column])
        y_valid = train_df[(train_df["train_heattm"] >= holdout_start) & (train_df["train_heattm"] <= holdout_end)][target_column] 
    else:
